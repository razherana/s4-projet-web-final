              ...
              <div class="loan-details-grid">
                ...
                <div class="loan-detail">
                  ...
                  <div class="detail-content">
                    ...
                    <div class="detail-value" id="monthly-payment-<?= $pret['id'] ?>">
                      <?php
                      // Calculate monthly payment to match admin logic
                      $montantMensuel = $pret['montant'] / ($pret['duree']);
                      $interetMensuel = ($pret['montant'] * $pret['taux_interet'] / 100) / ($pret['duree']);
                      $assuranceMensuelle = ($pret['montant'] * ($pret['taux_assurance'] ?? 0) / 100) / ($pret['duree']);
                      $paiementMensuel = $montantMensuel + $interetMensuel + $assuranceMensuelle;
                      echo number_format($paiementMensuel, 0, ',', ' ');
                      ?> Ar
                    </div>
                  </div>
                </div>
...
<script>
  ...
  function updateSummary(loan, schedule) {
    // Calculate monthly payment to match backend logic
    const montantMensuel = loan.montant / (loan.duree);
    const interetMensuel = (loan.montant * loan.taux_interet / 100) / (loan.duree);
    const assuranceMensuelle = (loan.montant * (loan.taux_assurance || 0) / 100) / (loan.duree);
    const paiementMensuel = montantMensuel + interetMensuel + assuranceMensuelle;
    ...
    <div class="summary-grid">
      ...
      <div class="summary-item">
        <div class="summary-value">${paiementMensuel.toLocaleString('fr-FR')} Ar</div>
  ...
  function updateTable(schedule, payments, loan) {
    ...
    schedule.forEach((p, i) => {
      if (p.isPayer) lastPaid = i + 1;
    });

    let html = `<table style="width: 100%; border-collapse: collapse;">
    <thead><tr style="background: #F8FAFC;">
      <th style="padding: 1rem; text-align: left;">Mois #</th>
    ...
    schedule.forEach((payment, i) => {
      ...
      const isPaid = payment.isPayer;

      if (isPaid) lastPaid = Math.max(lastPaid, monthNumber);

      const canPay = !isPaid && monthNumber <= lastPaid + 1;
      ...
      html += `<tr>
      <td style="padding: 1rem;">Mois ${monthNumber}</td>
